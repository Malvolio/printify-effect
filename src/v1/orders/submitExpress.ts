...
import { SubmitExpressOrderResponse } from "../schemas";
import { Address, ExpressLineItem, LineItem } from "../types";

export type SubmitExpressOrderResponseType = Schema.Schema.Type<
  typeof SubmitExpressOrderResponse
>;
 ...
 * @returns Effect that yields the express order submission response
 *
 * @example
 * const data = {
 ...
 *   },
 * };
 * const result = printify.orders.submitExpress(data);
 * // Expected response: {
 *     data: [
 ...
 *  }
 */
const submitExpress = (data: SubmitExpressData) => {
  return requestPrintify(
    SubmitExpressOrderResponse,
    `/v1/shops/{shopId}/express.json`,
    {
      method: "POST",
      data,
    }
